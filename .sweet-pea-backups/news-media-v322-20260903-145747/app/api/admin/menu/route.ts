...
const newsUpdateSchema = newsSchema.partial().refine((data) => Object.keys(data).length > 0, {
...
export async function PATCH(request: Request) {
  ...
  try {
    const body = (await request.json()) as { entity?: "item" | "category" | "shop" | "news"; id?: string; data?: unknown };
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if ((entity !== "item" && entity !== "category" && entity !== "news") || !id) {
      throw new Error("Yêu cầu xóa không hợp lệ.");
    }
