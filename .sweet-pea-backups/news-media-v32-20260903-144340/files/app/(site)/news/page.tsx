...
function NewsImage({ post, compact = false }: { post: NewsPost; compact?: boolean }) {
  ...
  return (
    <div className="soft-grid grid h-full place-items-center bg-[#c7db95] p-6">
      <Image
  ...
  return (
    <PageTransition>
      <section className="relative overflow-hidden border-b border-[#d9d1c0] bg-[#fffced]">
        <div className="pointer-events-none absolute -left-24 top-0 h-72 w-72 rounded-full bg-[#c7db95]/60 blur-3xl" />
        ...
        <div className="container-shell relative grid gap-8 py-10 sm:py-14 lg:grid-cols-[1fr_0.9fr] lg:items-center lg:gap-12 lg:py-16">
          <div className="max-w-2xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-[#d3d9b8] bg-[#c7db95] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.16em] text-[#496b54]">
              <Newspaper size={15} /> Bản tin Sweet Pea
            </span>
            <h1
              style={headingFont}
              className="mt-5 text-[clamp(3.25rem,7vw,6rem)] font-bold leading-[0.93] tracking-[-0.055em] text-[#184d39]"
            >
              Chuyện mới
            ...
            {featured ? (
              <Link href={`/news/${featured.id}`} className="group relative block overflow-hidden rounded-[1.6rem] bg-[#fffced]">
                ...
                <div className="absolute inset-x-3 bottom-3 rounded-[1.25rem] bg-[#184d39]/92 p-4 text-white shadow-lg backdrop-blur sm:inset-x-4 sm:bottom-4 sm:p-5">
                  <div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#c7db95]">
              ...
              <div className="grid aspect-[16/10] place-items-center rounded-[1.6rem] bg-[#c7db95] text-[#54705c]">
                Chưa có bản tin.
              </div>
            )}
          </div>
      ...
      <section className="bg-[#fffced] py-12 sm:py-16 lg:py-20">
        <div className="container-shell grid gap-8 lg:grid-cols-[18rem_minmax(0,1fr)] lg:items-start xl:gap-12">
          <aside className="order-2 grid gap-5 lg:order-1 lg:sticky lg:top-24">
            <section className="overflow-hidden rounded-[1.7rem] border border-[#ddd5c5] bg-[#fffced] shadow-[0_14px_40px_rgba(50,70,58,0.04)]">
              ...
              <div className="divide-y divide-[#eee7dc]">
                {latest.length ? latest.map((post) => (
                  <Link key={post.id} href={`/news/${post.id}`} className="group flex gap-3 p-4 transition hover:bg-white">
                    <div className="h-16 w-20 shrink-0 overflow-hidden rounded-xl border border-[#e2dacb] bg-[#c7db95]">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <h2 className="mt-1 line-clamp-2 text-sm font-bold leading-5 text-[#184d39] transition group-hover:text-[#64805a]">{post.title}</h2>
                    </div>
                  </Link>
            ...
            <section className="rounded-[1.7rem] border border-[#c7db95] bg-[#c7db95] p-5">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#7d9274]">Khám phá Sweet Pea</p>
              <nav className="mt-3 grid divide-y divide-[#c7db95] text-sm font-bold text-[#184d39]">
          ...
          <main className="order-1 min-w-0 lg:order-2">
            <div className="flex flex-col gap-3 border-b border-[#e4ddd0] pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 style={headingFont} className="mt-2 text-3xl font-bold tracking-[-0.035em] text-[#184d39] sm:text-4xl">Tất cả bản tin</h2>
              </div>
              <Link href="/menu" className="focus-ring inline-flex w-fit items-center gap-2 rounded-full border border-[#d6cdbd] bg-[#fffced] px-4 py-2.5 text-sm font-bold text-[#184d39]">
            ...
            {posts.length ? (
                  ...
                  <article key={post.id} className="group grid gap-5 py-7 sm:grid-cols-[15rem_minmax(0,1fr)] sm:items-center lg:grid-cols-[17rem_minmax(0,1fr)] lg:py-8">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {post.is_featured ? (
                          <span className="inline-flex items-center gap-1 rounded-full bg-[#c7db95] px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#51705a]">
                            <Sparkles size={11} /> Nổi bật
                          </span>
                        ) : null}
                      </div>
                      <h3 style={headingFont} className="mt-3 text-2xl font-bold leading-tight tracking-[-0.025em] text-[#184d39] sm:text-3xl">
                      ...
                      <Link href={`/news/${post.id}`} className="focus-ring mt-4 inline-flex items-center gap-2 text-sm font-extrabold text-[#184d39]">
              ...
              <div className="mt-7 rounded-[2rem] border border-dashed border-[#cdd5b4] bg-[#fffced] p-12 text-center text-[#6d7b72]">
