...
function RelatedImage({ post }: { post: NewsPost }) {
  ...
  return (
    <div className="soft-grid grid h-full place-items-center bg-[#e7edce] p-5">
  ...
  return (
    <PageTransition>
      <article className="bg-[#fffdf8]">
        <header className="relative overflow-hidden border-b border-[#ddd5c6] bg-[#f5efe3]">
          <div className="pointer-events-none absolute -left-20 top-10 h-64 w-64 rounded-full bg-[#dce7bd]/55 blur-3xl" />
          <div className="container-shell relative py-10 sm:py-14 lg:py-16">
            ...
            <div className="mt-8 max-w-4xl">
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#7d8e76]">
                ...
                {post.is_featured ? <span className="inline-flex items-center gap-1.5 rounded-full bg-[#e6edcd] px-2.5 py-1 text-[#55725b]"><Sparkles size={12} /> Nổi bật</span> : null}
              </div>
              <h1 style={headingFont} className="mt-4 text-[clamp(2.8rem,7vw,5.8rem)] font-bold leading-[0.98] tracking-[-0.055em] text-[#194d39]">{post.title}</h1>
        ...
        <div className="container-shell grid gap-9 py-10 sm:py-14 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start lg:gap-12 lg:py-16">
            ...
            <div className="overflow-hidden rounded-[2rem] border border-[#ddd5c5] bg-[#f1ebe0] p-2 shadow-[0_18px_55px_rgba(49,70,57,0.06)]">
              <div className="flex min-h-[16rem] items-center justify-center overflow-hidden rounded-[1.55rem] sm:min-h-[24rem]">
                {post.image_url ? (
                  ...
                  <div className="soft-grid grid min-h-[24rem] w-full place-items-center bg-[#e7edce]">
          ...
          <aside className="grid gap-4 lg:sticky lg:top-24">
            <div className="rounded-[1.6rem] border border-[#d8ddbf] bg-[#eef2da] p-5">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#7d9274]">Đọc tiếp</p>
              <h2 style={headingFont} className="mt-2 text-2xl font-bold text-[#244d3b]">Chuyện khác từ tiệm</h2>
            </div>
            {related.map((item) => (
              <Link key={item.id} href={`/news/${item.id}`} className="group overflow-hidden rounded-[1.5rem] border border-[#ddd5c5] bg-[#fffaf2]">
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-2 line-clamp-2 text-sm font-bold leading-5 text-[#244d3b]">{item.title}</h3>
