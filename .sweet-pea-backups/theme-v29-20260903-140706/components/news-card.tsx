...
export function NewsCard({ post, featured = false }: { post: NewsPost; featured?: boolean }) {
  return (
    <article className={`group overflow-hidden rounded-[2rem] border border-[#ddd5c5] bg-[#fffdf8] ${featured ? "md:grid md:grid-cols-[1.15fr_.85fr]" : ""}`}>
      <Link href={`/news/${post.id}`} className="block overflow-hidden bg-[#e5ebca]">
      ...
      <div className={`flex flex-col p-5 sm:p-7 ${featured ? "justify-center lg:p-10" : ""}`}>
        ...
        <h2 className={`font-display mt-3 font-semibold leading-tight tracking-[-0.035em] text-[#214e3d] ${featured ? "text-3xl sm:text-4xl" : "text-2xl"}`}>
        ...
        <Link href={`/news/${post.id}`} className="focus-ring mt-6 inline-flex w-fit items-center gap-2 rounded-full text-sm font-bold text-[#245943]">
